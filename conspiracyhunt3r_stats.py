...
def epub3thtml(epub_path):    
    ...
    for item in book.get_items():
        print(item)
    for item in book.get_items():
        if any(re.findall(r'Chapter2|Chapter02', str(item), re.IGNORECASE)):
           break
        print(item)
        if item.get_type() == ebooklib.ITEM_DOCUMENT:
           chapters.append(item.get_content())
...
shamingwords = [" icy ","ice","frozen","chill","hockey","arctic","Antarctica","Siberia","siberian","freeze","ice-cream","suck","sucking","sucks","windows","stick","finger","red","dark","child","children","nail","fetish","weird","strange","odd","follow","scent"]
