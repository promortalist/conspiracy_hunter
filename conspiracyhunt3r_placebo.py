...
icewords = [" icy ","ice","frozen","chill","hockey","arctic","Antarctica","Siberia","siberian","freeze","ice-cream","suck","sucking","sucks","cool","cold war","cold","fellatio","blowjob","oral sex","princess","queen","throat","air","breath","breathing","breaths","coffee","cafe"]

hotwords = ["hot", "burning", "fiery", "scalding", "very warm", "high temperature", "heated", "very warm", "oppressive", "heated","Irascible", "excitable", "impetuous", "passionate", "hasty", "choleric", "furious", "touchy", "violent"  "Ardent", "vehement", "fervent", "eager", "glowing", "animated", "fervid", "violent", "passionate", "pungent", "piquant", "sharp", "acrid", "biting", "stinging", "peppery", "high-flavored", "high-seasoned"]
...
def hotwords_test(word):
    i = 0
    counter1 = 0
    icedict = {}
    while i != len(hotwords):
         shamingword = hotwords[i]
         my_regex = r"\b(?=\w)" + re.escape(shamingword) + r"\b(?!\w)"
         if re.search(my_regex, str(word), re.IGNORECASE):
          # print("--------------------------------------------")
          # print("Found "+str(shamingword))
...
def ksiazka(epub): 
  pleroma = epub3text(epub)
  pleromastring = str(pleroma)
  pleromastr0 =  pleromastring.replace("et al.","et al")
  pleromastr =  pleromastr0.replace("\\n"," ")
  splitpleroma =  pleromastr.split(".")
  finalpleroma = [s + "." for s in splitpleroma]
  fragment = finalpleroma[0:500]
  metaicedict = {}
  for sentence in fragment:
      results = hotwords_test(sentence)
      icedict = results[1]
